    ...
    public partial class UploadPO : ProdataPage
    {
        ...
        private void BindData(string id)
        {
            using (var db = new AppDbContext())
            {
                ...
                // NEW: Bind PO Number
                txtPONumber.Text = form.PONumber.HasValue ? form.PONumber.Value.ToString() : "";
            ...
            using (var db = new AppDbContext())
            {
                using (var trans = db.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        decimal balance = (form.Amount ?? 0) - actualAmount;
                        ...
                        // NEW: Update PO Number (Safely parse to INT)
                        if (int.TryParse(txtPONumber.Text.Trim(), out int parsedPO))
                        {
                            form.PONumber = parsedPO;
                        }
                        else
                        {
                            form.PONumber = null; // Set null if empty or invalid
                        }
        ...
        private void BindAuditTrails(Guid formId)
        {
            ...
            if (auditTrails != null && auditTrails.Count > 0)
            {
                ...
                if (latestTrail != null && latestTrail.Action.Equals("Reviewed PO", StringComparison.OrdinalIgnoreCase))
                {
                    btnSave.Visible = false;
                    ApplyReadOnlyMode(); // 🚀 Trigger read-only lock!
                }
                else
                {
        ...
        // --- NEW: Locks the UI when btnSave is hidden ---
        private void ApplyReadOnlyMode()
        {
            // 1. Lock standard textboxes
            txtPONumber.Attributes.Add("readonly", "readonly");
            txtActualAmount.Attributes.Add("readonly", "readonly");

            // 2. Disable File Upload
            fuPO.Enabled = false;

            // 3. Hide the 'Add Row' button container
            if (divAddAllocationBtn != null) divAddAllocationBtn.Visible = false;

            // 4. Lock Savings Repeater textboxes
            foreach (RepeaterItem item in rptBudgetAllocations.Items)
            {
                var txt = item.FindControl("txtAllocateAmount") as TextBox;
                if (txt != null) txt.Attributes.Add("readonly", "readonly");
            }

            // 5. Lock Overrun Repeater textboxes and dropdowns
            foreach (RepeaterItem item in rptOverrunAllocations.Items)
            {
                var txt = item.FindControl("txtAllocationAmountItem") as TextBox;
                if (txt != null) txt.Attributes.Add("readonly", "readonly");

                var ddl = item.FindControl("ddlAllocationItem") as DropDownList;
                if (ddl != null) ddl.Enabled = false;
            }

            // 6. Use JavaScript to hide the red 'Remove' (-) buttons
            string script = "$(document).ready(function() { $('.btnRemoveAllocation').hide(); });";
            ScriptManager.RegisterStartupScript(this, GetType(), "LockUI", script, true);
        }
        private void BindAuditTrailsOld(Guid formId)
        {
            ViewState["pageIndex"] = ViewState["pageIndex"] ?? "0";
            var auditTrails = new Class.Approval().GetApprovals(formId, "Form");
            if (auditTrails != null && auditTrails.Count > 0)
