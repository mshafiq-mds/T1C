...
namespace Prodata.WebForm.Class
{
	public class Budget
	{
        ...
        // ===========================================
        public static string GetStatusName(string status, DateTime? deletedDate)
        {
            // Priority check for deleted status
            if (deletedDate != null) return "Deleted";

            // Since the DB now stores the string directly, simply return it.
            // Ensure we handle nulls gracefully.
            return !string.IsNullOrEmpty(status) ? status : "Unknown";
        }
        public static string GetStatusNameOld(int? status, DateTime? deletedDate)
        {
            if (deletedDate != null) return "Deleted";

            switch (status)
            {
                case 0: return "sentback";
        ...
        public static bool CanEditTransfer(TransferApprovalLimits limit, int userLevel, int currentLevel, DateTime? deletedDate, string status)
        {
            return deletedDate == null && limit != null && userLevel == currentLevel + 1 && (status != "Resubmit" || status != "Reject");
        }
        ...
        public List<Models.ViewModels.BudgetListViewModel> GetBudgetsIncludeGlobalBudget(
            Guid? entityId = null,
            Guid? typeId = null,
            int? year = null,
            int? month = null,
            ...
            Guid? excludedFormId = null)
        {
            using (var db = new AppDbContext())
            {
                ...
                if (!string.IsNullOrEmpty(bizAreaCode))
                    ...
                    query = query.Where(q => string.IsNullOrEmpty(q.BizAreaCode));
