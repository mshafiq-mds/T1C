...
{
    public partial class DashboardV2 : ProdataPage
    {
        ...
        private void BindModalGrid(string category, string statusType, string searchKeyword = "")
        {
            ...
            using (var db = new AppDbContext())
            {
                ...
                else if (category == "Additional")
                {
                    ...
                    totalAmount = 0;
                    gvDetails.DataSource = resultList.Select(x => new { RefNo = x.RequestNo, x.Date, BA = x.BA + " - " + (bizAreaDict.ContainsKey(x.BA) ? bizAreaDict[x.BA] : "-"), Status = x.Status.ToString(), Amount = "0.00" }).ToList();
                }
                // 4. Transfer Logic 
                else if (category == "Transfer")
                {
                    ...
                    totalAmount = 0; // Or Sum FromTransfer if logic applies
                    gvDetails.DataSource = resultList.Select(x => new { x.RefNo, x.Date, BA = x.BA + " - " + (bizAreaDict.ContainsKey(x.BA) ? bizAreaDict[x.BA] : "-"), Status = x.Status, x.Amount }).ToList();
                }
        ...
        private void LoadDashboardData()
        {
            ...
            using (var db = new AppDbContext())
            {
                ...
                // Declare count objects at the top so they can be accessed outside the try/catch blocks
                ...
                try
                {
                    ...
                    // Removed 'var' so it assigns to the pre-declared 'addS' variable
                ...
                try
                {
                    // 1. Pending Approvals (Sum of all "Under Review" / "Pending" statuses)
                    int totalPending = 
                ...
                // ----------------------------------------------------
                // DOUGHNUT CHART LOGIC: Top 10 BAs by Total T1C Amount
                // ----------------------------------------------------
                try
                {
        ...
        public string GetStatusBadgeClass(string status)
        {
            ...
            if (s.Contains("approve") || s.Contains("completed")) return "badge-approved";
            if (s.Contains("pending") || s.Contains("review")) return "badge-pending";
            if (s.Contains("resubmit") || s.Contains("reject") || s.Contains("deleted")) return "badge-resubmit";
